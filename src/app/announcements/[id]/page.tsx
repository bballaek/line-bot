...
export default function AnnouncementDetailPage() {
  ...
  if (!ann) return <div style={{ padding: 16, textAlign: "center", color: "#A1887F" }}>ไม่พบประกาศ</div>;

  const typeLabel = ann.pinned ? "แจ้งเพื่อดำเนินการ" : "แจ้งเพื่อทราบ";
  const typeColor = ann.pinned ? "#D97706" : "#F9A825";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FFF9F0", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, display: "flex", alignItems: "center", padding: "14px 16px", background: "rgba(255,249,240,0.95)", backdropFilter: "blur(10px)", borderBottom: "1px solid #F5E6D3" }}>
        ...
        <h1 style={{ flex: 1, textAlign: "center", fontSize: 17, fontWeight: 700, color: "#3E2723", margin: 0, paddingRight: 36 }}>ประกาศ</h1>
      </div>
      ...
      <div style={{ padding: "20px 16px", flex: 1, paddingBottom: 100 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #F5E6D3", padding: "20px 18px", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <Megaphone size={18} color="#FFC107" />
            <h2 style={{ fontSize: 18, fontWeight: 700, color: "#3E2723", margin: 0, flex: 1, lineHeight: 1.4 }}>{ann.title}</h2>
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
            <CalendarDays size={13} color="#A1887F" />
            <span style={{ fontSize: 12, color: "#A1887F" }}>กำหนดการณ์: {formatDateFull(ann.created_at)}</span>
          </div>
          ...
          <div style={{ borderTop: "1px solid #FFF9F0", paddingTop: 14 }}>
        ...
        {isCreator && (
          <button onClick={openStatus}
            ...
            <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <CheckCircle2 size={18} color="#FFC107" /> สถานะการรับทราบ
            </span>
        ...
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "12px 20px", paddingBottom: "calc(12px + env(safe-area-inset-bottom, 0px))", background: "rgba(255,249,240,0.95)", backdropFilter: "blur(10px)", borderTop: "1px solid #F5E6D3", zIndex: 100 }}>
          {hasRead ? (
            ...
            <button onClick={handleAcknowledge} disabled={acknowledging}
              style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", maxWidth: 400, margin: "0 auto", padding: 14, background: acknowledging ? "#A1887F" : "#FFC107", color: "#3E2723", fontSize: 15, fontWeight: 700, border: "none", borderRadius: 50, cursor: acknowledging ? "not-allowed" : "pointer" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 200, display: "flex", flexDirection: "column" }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#fff", marginTop: 60, borderRadius: "20px 20px 0 0", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", borderBottom: "1px solid #F5E6D3" }}>
              <div style={{ flex: 1, textAlign: "center", padding: "14px 0", fontWeight: 700, fontSize: 14, color: "#3E2723", borderBottom: "3px solid #FFC107" }}>
            ...
            <div style={{ flex: 1, overflow: "auto", padding: "20px 16px" }}>
              {loadingStatus ? (
                ...
                <div style={{ textAlign: "center", padding: "40px 0", color: "#A1887F", fontSize: 14 }}>ยังไม่มีคนรับทราบ</div>
              ) : (
