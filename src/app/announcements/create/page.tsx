...
export default function CreateAnnouncementPage() {
  ...
  return (
    <TeacherOnlyGate title="สร้างประกาศ" backHref="/announcements" featureLabel="ประกาศ">
    <div style={{ minHeight: "100vh", background: "#FFF9F0", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, display: "flex", alignItems: "center", padding: "14px 16px", background: "rgba(255,249,240,0.95)", backdropFilter: "blur(10px)", borderBottom: "1px solid #F5E6D3" }}>
        <button onClick={() => (window.location.href = "/announcements")} style={{ background: "none", border: "none", cursor: "pointer", padding: "4px 8px", color: "#5D4037", display: "flex", alignItems: "center" }}>
          <ArrowLeft size={22} />
        </button>
        <h1 style={{ flex: 1, textAlign: "center", fontSize: 17, fontWeight: 700, color: "#3E2723", margin: 0, paddingRight: 36 }}>สร้างประกาศ</h1>
      </div>

      {/* Image Upload */}
      <input type="file" accept="image/*" ref={fileInputRef} onChange={handleImageUpload} style={{ display: 'none' }} />
      {imageUrl ? (
        <div style={{ margin: "16px 16px 0", borderRadius: 14, overflow: "hidden", position: "relative", border: "1px solid #F5E6D3" }}>
        ...
        <div style={{ margin: "16px 16px 0", borderRadius: 14, background: "#FFF8E1", padding: "28px 20px", textAlign: "center", border: "1px dashed #FFE082" }}>
          <Upload size={36} color="#F9A825" style={{ marginBottom: 10 }} />
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
            style={{ background: "#FFC107", color: "#3E2723", border: "none", borderRadius: 50, padding: "8px 20px", fontSize: 13, fontWeight: 600, cursor: uploading ? "not-allowed" : "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>
            {uploading ? <><span>กำลังอัปโหลด...</span></> : <><ImageIcon size={14} /> อัปโหลดภาพ</>}
          </button>
          <div style={{ fontSize: 11, color: "#795548", marginTop: 6 }}>ไฟล์รูปภาพขนาดไม่เกิน 5MB</div>
        </div>
      )}

      {/* Form */}
      <div style={{ padding: "20px 16px", flex: 1, paddingBottom: 100 }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, color: "#3E2723", marginBottom: 16 }}>ข้อมูลประกาศ</h2>
        ...
        <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {!showPreview && (
                <div style={{ display: "flex", gap: 4, background: "#FFF9F0", padding: "2px", borderRadius: 6 }}>
                  ...
                  <button onClick={(e) => {e.preventDefault(); insertFormat('link');}} style={{ background: "none", border: "none", padding: 4, cursor: "pointer", borderRadius: 4 }} title="รายการ"><Link2 size={14} color="#5D4037" /></button>
                </div>
              )}
              <button 
          ...
          {showPreview ? (
            <div style={{ ...inputStyle, minHeight: 120, border: "1px solid #F5E6D3", background: "#FFFBF5" }}>
              {content ? <MarkdownRenderer content={content} /> : <span style={{ color: "#A1887F", fontSize: 14 }}>ยังไม่มีข้อความ...</span>}
            </div>
          ) : (
            <>
              ...
              <div style={{ textAlign: "right", marginTop: 4, fontSize: 12, color: "#A1887F" }}>
                รอบรับการจัดรูปแบบด้วย <a href="https://www.markdownguide.org/cheat-sheet/" target="_blank" rel="noreferrer" style={{ color: "#FFC107", textDecoration: "none" }}>Markdown</a> ({content.length}/{MAX_CONTENT})
              </div>
            </>
          )}
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={labelStyle}><Link2 size={14} /> ลิงก์แนบ <span style={{ fontWeight: 400, color: "#A1887F" }}>(เช่น เว็บไซต์อ้างอิง)</span></label>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ border: "1px dashed #D7CCC8", borderRadius: 12, padding: "24px 20px", textAlign: "center", background: "#FFFBF5" }}>
            <Upload size={24} color="#A1887F" style={{ marginBottom: 6 }} />
            <div style={{ fontSize: 13, color: "#795548", fontWeight: 500 }}>อัปโหลดไฟล์</div>
          ...
          <div style={{ display: "flex", gap: 20 }}>
            {[{ val: "info" as const, icon: <FileEdit size={14} />, label: "แจ้งเพื่อทราบ" },
              ...
              <label key={opt.val} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 14, color: "#3E2723" }}>
                <div onClick={() => setAnnType(opt.val)} style={{ width: 20, height: 20, borderRadius: "50%", border: annType === opt.val ? "none" : "2px solid #D7CCC8", background: annType === opt.val ? "#FFC107" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
      ...
      {showPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: "28px 24px", maxWidth: 360, width: "100%", position: "relative" }}>
            <button onClick={() => { resetForm(); window.location.href = "/announcements"; }} style={{ position: "absolute", top: 12, right: 12, background: "none", border: "none", cursor: "pointer", color: "#A1887F" }}><X size={20} /></button>
