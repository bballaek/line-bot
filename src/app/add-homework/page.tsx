      ...
      <div style={{ padding: "20px 16px", flex: 1, paddingBottom: 100 }}>
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, padding: "0 4px" }}>
            <label style={{ ...labelStyle, marginBottom: 0 }}><AlignLeft size={16} /> รายละเอียดการบ้าน</label>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              {!showPreview && (
                <div style={{ display: "flex", gap: 4 }}>
                  ...
                  <button onClick={(e) => {e.preventDefault(); insertFormat('link');}} style={{ background: "none", border: "none", padding: 4, cursor: "pointer" }} title="ลิงก์"><Link2 size={16} color="#475569" /></button>
                </div>
              )}
              <button
          ...
          {showPreview ? (
            <div style={{ ...inputStyle, minHeight: 120, border: "1px solid #E2E8F0", background: "#F8FAFC" }}>
              {description ? <MarkdownRenderer content={description} /> : <span style={{ color: "#94A3B8", fontSize: 14 }}>ยังไม่มีข้อความ...</span>}
            </div>
          ) : (
            <>
              ...
              <div style={{ textAlign: "right", marginTop: 4, fontSize: 12, color: "#94A3B8" }}>
                รองรับ <a href="https://www.markdownguide.org/cheat-sheet/" target="_blank" rel="noreferrer" style={{ color: "#2563EB", textDecoration: "none" }}>Markdown</a> ({description.length}/{MAX_DESC})
              </div>
            </>
          )}
        </div>
